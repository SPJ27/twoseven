...
export default function LoginPage() {
  ...
  const signIn = async () => {
    await supabase.auth.signInWithOAuth({
      ...
      options: {
        redirectTo:
          "https://twoseven.sakshamjain.dev/auth/callback?next=/dashboard",
      },
    });
  };
  ...
  return (
    <div
      className={`${dmSans.className} min-h-screen flex items-center justify-center bg-[#f7f6f2]`}
    >
      <div className="flex w-full max-w-3xl rounded-2xl overflow-hidden shadow-sm border border-black/[0.06]">
        ...
        <div className="flex-1 bg-[#0f0f0e] p-14 flex flex-col justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className={`${dmSerif.className} text-[#f5f0e8] text-2xl`}>
              TwoSeven
            </span>
          </div>
          <h1
            className={`${dmSerif.className} text-[#f5f0e8] text-4xl font-light leading-tight`}
          >
            Data that
            <br />
            <em className="text-[#c9b99a]">is reliable.</em>
            <br />
            Better.
          </h1>
        ...
        <div className="flex-1 bg-white p-14 flex flex-col justify-center">
          <h2 className={`${dmSerif.className} text-2xl text-gray-900 mb-1`}>
            Welcome back
          </h2>
